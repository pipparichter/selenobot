...
class Classifier(torch.nn.Module):
    ...
    def fit(self, train_dataset, val_dataset, epochs:int=10, lr:float=0.01, batch_size:int=16):
        ...
        val_accs = [balanced_accuracy_score(val_dataset.labels.cpu().numpy(), self.predict(val_dataset).cpu().numpy())]
        ...
        for epoch in range(epochs):
            ...
            val_accs.append(balanced_accuracy_score(val_dataset.labels.cpu().numpy(), self.predict(val_dataset).cpu().numpy()))
