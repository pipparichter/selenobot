...
import random
import os
from selenobot.utils import fasta_concatenate, pd_to_fasta, pd_from_fasta, fasta_size
...
def setup_sec_truncated(config:ConfigParser, first_sec_only:bool=True) -> ConfigParser:
    ...
    df = pd_from_fasta(config['paths']['sec_path'], set_index=False)
...
def setup(config:ConfigParser) -> ConfigParser:
    ...
    fasta_concatenate([config['paths']['sec_truncated_path'], config['paths']['sprot_path']], out_path=all_data_path)
